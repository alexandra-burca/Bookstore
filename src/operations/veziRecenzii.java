 ...
 *
 */
public class veziRecenzii extends JFrame {
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    veziRecenzii frame = new veziRecenzii();
    ...
    {
        ArrayList<Recenzie> recenzieList = new ArrayList<Recenzie>();
        Connection dbconn = DBConnection.connectDB();

        String query = "SELECT `username`, `bookID`, `rating`, `textrecenzie` FROM  recenzii";
        Statement st;
        ResultSet rs;
        ...
        try {
            ...
            while(rs.next())
            {
                recenzie = new Recenzie(rs.getString("username"),rs.getInt("bookID"),rs.getInt("rating"),rs.getString("textrecenzie"));
    ...
    public void ShowTable() {
        ArrayList<Recenzie> list = getRecenzii();
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            row[3] = list.get(i).getTextrecenzie();
    ...
    public void initialize() {
        ...
        lblNewLabel_1.setIcon(new ImageIcon("C:\\Users\\Alexandra\\IdeaProjects\\Bookstore\\icons\\popular.png"));
        lblNewLabel_1.setBounds(267, 127, 50, 50);
